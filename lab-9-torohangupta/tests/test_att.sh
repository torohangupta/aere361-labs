...
if [ -s $ATT_DATA ];
	then
		tput setaf 2
  	echo -e "Checking that $ATT_DATA is not empty....OK"
    if grep -q "0	 -0.19	 -2.01	 -8.12" $ATT_DATA;
    then
			tput setaf 2
      echo -e "Validating that proper data is in $ATT_DATA....OK"
    else
			tput setaf 1
      echo -e "Validating that proper data is in $ATT_DATA....FAILED"
      exit 1
		fi
else
		tput setaf 1
    echo -e "Checking that $ATT_DATA is not empty....FAILED"
    exit 1
fi
