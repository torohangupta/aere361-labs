...
if [ -s $FILESRC ];
    ...
    if grep -q "malloc" $FILESRC && grep -q "sizeof" $FILESRC;
    then
      echo -e "Validating that malloc and sizeof is in $FILESRC....OK"
    else
      echo -e "Validating that malloc and sizeof is in $FILESRC....FAILED"
      exit 1
		fi
else
