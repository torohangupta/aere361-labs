...
if [ -s $FILEOBJ_CSVPARSE_STATIC ];
	then
		tput setaf 2
  	echo -e "Checking that $FILEOBJ_CSVPARSE_STATIC is not empty....OK"
    
else
		tput setaf 1
    echo -e "Checking that $FILEOBJ_CSVPARSE_STATIC is not empty....FAILED"
		make clean
    exit 1
fi
...
if [ -s $FILEA_CSVPARSE ];
	then
		tput setaf 2
  	echo -e "Checking that $FILEA_CSVPARSE is not empty....OK"
    
else
		tput setaf 1
    echo -e "Checking that $FILEA_CSVPARSE is not empty....FAILED"
		make clean
    exit 1
fi
...
# Test that proper output file is generated
if [ -s $FILEOBJ_CSVPARSE_STATIC ];
	then
  	echo -e "Checking that $FILEOBJ_CSVPARSE_STATIC is not empty....OK"
