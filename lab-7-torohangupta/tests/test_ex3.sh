...
if [ -s $FILEOBJ_ADD_SHARED ];
	then
		tput setaf 2
  	echo -e "Checking that $FILEOBJ_ADD_SHARED is not empty....OK"
    
else
		tput setaf 1
    echo -e "Checking that $FILEOBJ_ADD_SHARED is not empty....FAILED"
    exit 1
fi

if [ -s $FILEOBJ_ANSWER_SHARED ];
	then
		tput setaf 2
  	echo -e "Checking that $FILEOBJ_ANSWER_SHARED is not empty....OK"
    
else
		tput setaf 1
    echo -e "Checking that $FILEOBJ_ANSWER_SHARED is not empty....FAILED"
    exit 1
fi
...
if [ -s $FILESO_ADD ];
	then
		tput setaf 2
  	echo -e "Checking that $FILESO_ADD is not empty....OK"
    
else
		tput setaf 1
    echo -e "Checking that $FILESO_ADD is not empty....FAILED"
    exit 1
fi
